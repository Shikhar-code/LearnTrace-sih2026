...
interface StatCardProps {
  ...
  icon?: LucideIcon;
  trend?: string;
  colorScheme?: 'teal' | 'emerald' | 'amber' | 'stone' | 'indigo' | 'blue';
}
  ...
  return (
    <div className="bg-white border border-stone-200/80 rounded-xl p-5 shadow-xs flex items-center justify-between transition-all hover:border-stone-300">
      <div>
        ...
        <p className="text-2xl font-bold text-stone-900 mt-1 tracking-tight">{value}</p>
        {subtitle && <p className="text-xs text-stone-500 mt-0.5">{subtitle}</p>}
        {trend && <p className="text-xs text-emerald-700 font-medium mt-1">{trend}</p>}
      </div>
      {Icon && (
