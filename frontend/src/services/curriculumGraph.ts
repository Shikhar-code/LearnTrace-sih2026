...
export const conceptId = (classLevel: number, subject: string, chapter: string): string => {
...
export const CURRICULUM_DEPENDENCIES: Array<[number, string, string, string, number]> = [
  // Class 9 Mathematics
...
export const buildBaselineCurriculumGraph = (
  ...
  const nodeMap = new Map<string, { id: string; label: string; parents: string[] }>();
  ...
  for (const [lvl, sub, conceptName, prereqName, weight] of CURRICULUM_DEPENDENCIES) {
  ...
  const targetNode = nodeMap.get(defaultTarget) || Array.from(nodeMap.values())[0];
